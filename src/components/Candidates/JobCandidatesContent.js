...
const JobCandidatesContent = () => {
  ...
  }, [searchTerm, selectedStatus, jobId]);
  ...
  return (
    <div className={`space-y-6 p-6 ${colors.bg} transition-colors duration-300`}>
      <Card className={`w-full ${isDark ? 'bg-gray-800 shadow-xl' : 'bg-white shadow-md'} transition-all duration-300`}>
        <CardHeader className={`flex justify-between items-center ${isDark ? 'border-b border-gray-700' : 'border-b border-gray-100'}`}>
          <div>
            <CardTitle className={colors.text}>Job Applications</CardTitle>
          ...
          <button
            ...
            className={`px-4 py-2 text-sm ${isDark ? 'bg-gray-700 hover:bg-gray-600 text-gray-200' : 'bg-gray-100 hover:bg-gray-200 text-gray-800'} rounded-lg transition-colors duration-200`}
          >
            ...
            <table className={`min-w-full divide-y ${colors.border}`}>
              ...
              <tbody className={`${colors.bgCard} divide-y ${colors.border}`}>
                {applications.length > 0 ? (
                  ...
                  <tr>
                    <td colSpan="5" className={`px-6 py-10 text-center ${colors.textMuted}`}>
