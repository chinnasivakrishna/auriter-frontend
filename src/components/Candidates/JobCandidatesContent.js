...
const JobCandidatesContent = () => {
  ...
  useEffect(() => {
    if (searchTerm || selectedStatus !== 'all' || selectedJobType !== 'all') {
      const timer = setTimeout(() => {
        searchApplications();
      }, 500);
      return () => clearTimeout(timer);
    }
  }, [searchTerm, selectedStatus, selectedJobType, jobId]);

  const fetchAnalysis = async (application) => {
    try {
      const token = Cookies.get('token');
      console.log('Fetching analysis for application:', application._id);
    ...
    try {
      ...
      data.applications.forEach(application => {
        console.log('Application:', {
          ...
          interviewRoomId: application.interviewRoomId
        });
      });
      ...
      setStats({
        ...
        reviewed: data.applications.filter(app => app.status === 'reviewed').length,
        shortlisted: data.applications.filter(app => app.status === 'shortlisted').length,
  ...
  const handleSendInterviewLink = (application) => {
    console.log('Selected application for interview:', application);
  ...
  const handleInterviewSubmit = async () => {
    try {
      const token = Cookies.get('token');
      console.log('Submitting interview details:', {
      ...
      // Refresh the applications to get updated interview status
      fetchApplications();
    } catch (err) {
  ...
  const handleViewResults = (applicationId) => {
    console.log('Viewing results for application:', applicationId);
  ...
  // Return the same JSX as CandidatesContent but with the added back button
  return (
          ...
          <CardTitle>Job Applications</CardTitle>
            {applications.length > 0 && (
              <p className="text-sm text-gray-500 mt-1">
                Showing {applications.length} applications for {applications[0]?.job?.title}
              </p>
            )}
          </div>
          <button
...
export default JobCandidatesContent; 